# GitHub Features

- ***Repositories*** - Store and manage your project code.
- ***Branches*** - Work on features or fixes without affecting main code.
- ***Pull Requests(PRs)*** - Propose and review code changes before merging.
- ***Issues*** - Track bugs, tasks, and feature requests.
- ***GitHub Actions*** - Automate CI/CD workflows (build, test, deploy).
- ***GitHub Pages*** - Host static websites directly from a repository.
- ***Projects*** - Kanban-style boards to manage tasks and progress.
- ***Wiki*** - Documentation space for your repository.